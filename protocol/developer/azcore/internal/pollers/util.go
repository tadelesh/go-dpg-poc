...
func ResultHelper[T any](resp *http.Response, failed bool, out *T) error {
	...
	if reflect.TypeOf(out).Kind() == reflect.Pointer && reflect.TypeOf(out).Elem().Kind() == reflect.Slice && reflect.TypeOf(out).Elem().Elem().Kind() == reflect.Uint8 {
		reflect.ValueOf(out).Elem().Set(reflect.ValueOf(payload))
	} else {
